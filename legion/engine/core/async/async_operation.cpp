...
{
    float async_progress::size() const noexcept
    {
        return static_cast<float>(m_size) / precision_scale<float>;
    }

    size_type async_progress::rawSize() const noexcept
    {
        return m_size;
    }
    ...
    void async_progress::advance_progress(float progress) noexcept
    {
    ...
    async_operation_base::async_operation_base(const std::shared_ptr<async_progress>& progress) noexcept
        : m_progress(progress)
    {
    }
