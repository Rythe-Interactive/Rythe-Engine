...
namespace legion::core::compute
{
        ...
        , m_size(0)
    {
        ...
        if (ret != CL_SUCCESS)
        {
            log::error("clCreateFromGLTexture failed for Buffer: {}", name);
        }
        ...
        , m_size(0)
    {
        ...
        if (ret != CL_SUCCESS)
        {
            log::error("clCreateFromGL(X?+)Buffer failed for Buffer: {}", name);
        }
    }
    ...
    Buffer::Buffer(cl_context ctx, byte* data, size_type len, buffer_type type, std::string name) :m_size(len), m_data(data), m_name(std::move(name))
    {
