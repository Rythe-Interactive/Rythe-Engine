...
#pragma region contains
		...
		{
			return m_sparse[val] >= 0 && m_sparse[val] < m_dense.size() && m_dense[m_sparse[val]] == val;
		}
		...
		{
			return m_sparse.count(val) && m_sparse.at(val) >= 0 && m_sparse.at(val) < m_dense.size() && m_dense[m_sparse.at(val)] == val;
		}
