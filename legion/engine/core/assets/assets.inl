...
namespace legion::core::assets
{
    ...
    inline common::result<asset<AssetType>> AssetCache<AssetType>::createInternal(id_type nameHash, const std::string& name, id_type loaderId, Args && ...args)
    {
        ...
        m_data.m_info.try_emplace(nameHash, detail::asset_info{ name, loaderId });
    ...
    inline common::result<asset<AssetType>> AssetCache<AssetType>::retry_load(common::result<asset<AssetType>> previousAttempt, id_type previousLoader, id_type nameHash, const std::string& name, const fs::view& file, const AssetCache<AssetType>::import_cfg& settings)
    {
        ...
        if (loader)
        {
            auto result = loader->load(nameHash, name, file, settings);
            if (result)
                return result;
    ...
    inline void AssetCache<AssetType>::addLoader(Arguments&& ...args)
    {
        ...
        ptr->m_loaderId = m_data.m_loaders.size() + 1;

        m_data.m_loaderIds.emplace(typeHash<LoaderType>(), ptr->m_loaderId);
        m_data.m_loaders.emplace_back(ptr);
    }

    template<typename AssetType>
    template<typename LoaderType, typename ...Arguments>
    inline asset<AssetType> AssetCache<AssetType>::createAsLoader(id_type nameHash, const std::string& name, Arguments && ...args)
    {
        ...
        m_data.m_info.try_emplace(nameHash, detail::asset_info{ name, m_data.m_loaderIds.at(typeHash<LoaderType>()) });
        return asset<AssetType>{ ptr, nameHash };
    }
    ...
    inline asset<AssetType> AssetCache<AssetType>::create(id_type nameHash, const std::string& name)
    {
        ...
        m_data.m_info.try_emplace(nameHash, { name, invalid_id });
        return { ptr, nameHash };
    }
    ...
    inline asset<AssetType> AssetCache<AssetType>::create(id_type nameHash, const std::string& name, Arguments&&... args)
    {
        ...
        AssetType* ptr = &m_data.m_cache.try_emplace(nameHash, AssetType(std::forward<Arguments>(args)...)).first->second;
        m_data.m_info.try_emplace(nameHash, detail::asset_info{ name, invalid_id });
        return { ptr, nameHash };
    }
