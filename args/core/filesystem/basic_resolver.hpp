...
namespace args::core::filesystem
{
    ...
    public:
        ...
        A_NODISCARD bool is_file() const noexcept override
        {
            return !is_directory() && is_valid();
        }
        A_NODISCARD bool is_directory() const noexcept override
        {
        ...
        A_NODISCARD bool is_valid() const noexcept override
        {
        ...
        A_NODISCARD bool writeable() const noexcept override
        {
        ...
        A_NODISCARD bool readable() const noexcept override
        {
        ...
        A_NODISCARD bool creatable() const noexcept override
        {
            return !exists() && writeable();
        }
        A_NODISCARD bool exists() const noexcept override
        {
            return std::filesystem::exists(strpath_manip::subdir(m_root_path,get_target()));
        }
        A_NODISCARD std::set<std::string> ls() const noexcept override{ return {}; }
        common::result<basic_resource, fs_error> get(interfaces::implement_signal_t) const noexcept override
        {
            using common::Err, common::Ok;

            if(!exists()) return Err(args_fs_error("file does not exist cannot read"));
            if(!is_file()) return Err(args_fs_error("not a file"));
            if(!readable()) return Err(args_fs_error("file not readable"));
        ...
        common::result<void,fs_error> set(interfaces::implement_signal_t, const basic_resource& res) override
        {
            ...
            if(!writeable()) return Err(args_fs_error("file not writeable"));
            if(is_directory())
            {
                if(!res.empty()) return Err(args_fs_error("attempted to create directory with data!"));
                std::error_code code;
                std::filesystem::create_directories(full,code);
                if(code.value() != 0)
                {
                    return Err(args_fs_error(("std::filesystem bailed! " + code.message()).c_str()));
                }
                return Ok();
            }
