...
namespace args::core
{
    ...
    public:
        A_NODISCARD async::readonly_rw_spinlock& get_lock() const { return m_container_lock; }
        ...
        A_NODISCARD const_iterator cbegin() const
        {
            async::readonly_guard lock(m_container_lock);
            return const_iterator(m_dense_key.cbegin(), m_dense_value.cbegin());
        }
        ...
        A_NODISCARD const_iterator cend() const
        {
            async::readonly_guard lock(m_container_lock);
            return const_iterator(m_dense_key.cbegin() + m_size, m_dense_value.cbegin() + m_size);
        }
         ...
         */
        A_NODISCARD inline size_type size(std::memory_order order = std::memory_order_acquire) const noexcept { return m_size.load(order); }
         ...
         */
        A_NODISCARD inline size_type capacity(std::memory_order order = std::memory_order_acquire) const noexcept { return m_capacity.load(order); }
         ...
         * @returns size_type
         */
        A_NODISCARD size_type max_size() const noexcept { async::readonly_guard lock(m_container_lock); return m_dense_value.max_size(); }

        /**@brief Returns whether the sparse_map is empty.
         * @returns bool True if the sparse_map is empty, otherwise false.
         */
        A_NODISCARD inline bool empty(std::memory_order order = std::memory_order_acquire) const noexcept { return m_size.load(order) == 0; }
