...
#pragma endregion
    ...
    void onPlayerLookX(player_look_x* action)
    {
        ...
        rotH.fetch_multiply(math::angleAxis(action->value * action->input_delta * 100.0f, math::vec3(0, 1, 0)));
    ...
    {
        auto rotH = player.get_component_handle<rotation>();
        rotH.fetch_multiply(math::angleAxis(action->value * action->input_delta * 100.0f, math::vec3(1, 0, 0)));
        rotH.read_modify_write(rotation(), [](const rotation& src, rotation&& dummy)
            {
                (void)dummy;
                math::vec3 fwd = math::toMat3(src) * math::vec3(0.f, 0.f, 1.f);
