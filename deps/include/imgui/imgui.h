...
struct ImVec2
{
    ...
    float  operator[] (size_t idx) const    { return idx == 0 ? x : y; }    // We very rarely use this [] operator, the assert overhead is fine.
    float& operator[] (size_t idx)          { return idx == 0 ? x : y; }    // We very rarely use this [] operator, the assert overhead is fine.
